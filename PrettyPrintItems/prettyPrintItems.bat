...
:: *    @li You can have many <tt>SyMenuItems.zip</tt> placed anywhere (if you use <tt>-fc</tt> or <tt>-fp</tt> <A href="https://www.ugmfree.it/SyMenuManual.aspx#CommandLine">parameters</A> )@n
...
:: *  so I added the file <tt>prettyPrintItems.hide.vbs</tt>.  This little VBS program is general purpose: it runs hidden any BAT file passed as parameter. 
...
:: *                    &lt;param>
:: *                         "prettyPrintItems.hide.vbs" "prettyPrintItems.bat"
:: *                    &lt;/param>
:: *              &lt;/SyProgramCmd>
:: *
:: *              &lt;SyProgramCmd>
...
:: *                    &lt;path>wscript.exe&lt;/path>
:: *                    &lt;param>
:: *                        "prettyPrintItems.hide.vbs" "prettyPrintItems.bat"  "./../../config MM/SyMenuItems.zip"]
:: *                    &lt;/param>
:: *               &lt;/SyProgramCmd>
:: *               </pre> </code>
:: *    Note:
:: *          @li The program <tt>Path</tt>  MUST be <b>wscript.exe</b> (only the file name, don't use quotes here)
:: *         @li The parameters MUST be: the VBS file and the BAT file. If required add the <tt>SyMenuItems.zip</tt> pat. You can also use absolute paths, if you like.
:: *          @li Use double quotes around the paths in the <i>parameter section</i>.
:: *          @li If you plan to use <tt>tprettyPrintItems.hide.vbs</tt> for other BAT files, a caveat: all arguments sent to BAT file are in doubles quotes.
:: *
:: *  @par Output
...
:: *
:: *  @par parameter
:: *   Two parameters controls some aspects of the output HTML page:
:: *   @li  <tt>status</tt>: defines the initial look of tree: open (default) or close.
...
:: *
:: *   You can change the defaults editing the last line of code. For more info see 
:: *
:: *  @par customization
...
:: *   @li You first thig is to insert in target page the CSS and Javascrip blocs. This can be done two ways:@n
...
:: *
:: * @par acknowledgments
:: *    @li <A href="https://www.7-zip.org/download.html">7z.exe</A> used to unzip the configuration file.
...
:: *
:: *  This program is a filter thant uses <tt>xmlfilter.java</tt> with a XSL files 'ad hoc': xmlPrintSyItems.xslt.
:: *  The program get a snapshot of the actual SyMenuItems.zip and produce3s a HTML tree presentation, colored and collassable.
:: *  @par How it works
:: *     -# STEP: checks the paths and setup some vars (BATPATH and CONFIGFILE). If any error, die.
:: *       If the <code>SyMenuItem.zip</code> is't in his default directory, the user MUST add its actual path as parameter.
